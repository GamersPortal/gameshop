    ...
    <div class="panel-body white-section">
      <ul class="list-group">
        @foreach($cart->content() as $product)
          <li class="list-group-item">{{ $product->name }}, x{{ $product->qty }},
            ${{ $product->price * $product->qty }} </li>
        @endforeach
        @if( Cart::total() < 150)
    ...
    <div class="panel-footer clearfix">
      <span class="total-price"><strong>Total: ${{ Cart::totalWithShipping() }}</strong></span>
		<span class="pull-right">
			<a href="{{ route('StoreOrder') }}">
        <button class="btn btn-success">Proceed with order</button>
      </a>
			<a href="/">
